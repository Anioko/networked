...
@organisations.route('/list/')
def org_list():
    appts = Organisation.query.all()
    return render_template('organisations/allorganisations.html', appts=appts)
...
def select_org(org_id):
    ...
    if current_user.id != org.user_id and current_user not in org.get_staff():
        abort(404)
    return render_template('organisations/org_operations.html', org=org)


@organisations.route('/<org_id>/<org_name>')
def public_org(org_id, org_name):
    org = Organisation.query.filter(Organisation.id==org_id).first_or_404()
    return render_template('organisations/org_public.html', org=org)
...
@organisations.route('/<org_id>/list_events', methods=['Get', 'POST'])
@login_required
def list_events(org_id):
    org = Organisation.query.filter_by(id=org_id).first_or_404()
    if current_user.id != org.user_id and current_user not in org.get_staff():
        abort(404)
    events = Event.query.filter_by(organisation_id=org_id).all()
    return render_template('organisations/list_events.html', events=events, org=org)
...
@organisations.route('/<org_id>/list_services', methods=['Get', 'POST'])
@login_required
def list_services(org_id):
    org = Organisation.query.filter_by(id=org_id).first_or_404()
    if current_user.id != org.user_id and current_user not in org.get_staff():
        abort(404)
    services = Service.query.filter_by(organisation_id=org_id).all()
    return render_template('organisations/list_services.html', services=services, org=org)
...
def view_submissions(id):
    ...
    if promo is None:
        ...
        submission_ids = [appt.user_id for appt in submissions]
        submissions = [User.query.get(user_id) for user_id in submission_ids]
...
@organisations.route('/<int:id>/attendees/')
@login_required
def view_attendees(id):
    ...
    if event is None:
        ...
        abort(403)
    elif event.creator_id != current_user.id:
        abort(403)
    else:
        #applicants_profiles = {}
        attendees = Attendee.query.filter(Attendee.event_id == id).all()
        attendees_ids = [appt.user_id for appt in attendees]
        attendees = [User.query.get(user_id) for user_id in attendees_ids]
        return render_template('organisations/attendees.html', id=id,
                               attendees=attendees)
